...
def check_argv():
    ...
    parser.add_argument(
        "--word_type", type=str,
        help="show a plot for these word types, given as comma-seperated "
        "values"
        )
    parser.add_argument(
        "--plot_rnd", type=int,
        help="plot this number of randomly selected embeddings"
        )
    parser.add_argument(
        "--plot_all", action="store_true", help="plot all embeddings"
        )
    parser.add_argument(
        "--normalise", dest="normalise", action="store_true",
        help="normalise embeddings to unit sphere before calculating "
        "distances (default is not to do this)"
        )
