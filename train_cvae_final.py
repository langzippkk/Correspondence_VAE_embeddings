...
default_options_dict = {
        ...
        ##################################
        "enc_n_hiddens": [300, 300, 300],
        "dec_n_hiddens": [300, 300, 300],  
        ##################################
        "n_z": 130,                         # latent dimensionality
        ##################################
        "n_sample": 5, # number of samples of z, we only pick one that results the biggest ELBO
        ##################################
        "sigma_sq": 0.00001,
        ...
        "ae_n_epochs": 0,                 # AE pretraining options
        ##################################
        "ae_batch_size": 300,
        "ae_n_buckets": 3,
        "pretrain_usefinal": False,         # if True, do not use best
                                            ...
                                            # use final model
        ##################################
        "cae_n_epochs": 70,                 # CAE training options
        # "cae_n_epochs": 2,
        ##################################
        "cae_batch_size": 300,
        "cae_n_buckets": 3,
...
def build_cae_from_options_dict(a, a_lengths, b_lengths, options_dict):
    ...
    ####################
    build_latent_func = tflego.build_vae_new
    ####################
    ...
    latent_func_kwargs = {
        "enc_n_hiddens": [],
        "n_z": options_dict["n_z"],
        "n_sample": options_dict["n_sample"],
        "dec_n_hiddens": [options_dict["dec_n_hiddens"][0]],
        "activation": tf.nn.relu
        }
    ########################################
    ...
    # Network
    network_dict = tflego.build_multi_encdec_lazydynamic_latentfunc_new(
    ...
    if options_dict["d_speaker_embedding"] is not None:
        return {
            "z": z, "y": y, "mask": mask, "speaker_id": speaker_id,
            "speaker_embedding": speaker_embedding, "z_mean": z_mean, "z_log_sigma_sq":z_log_sigma_sq
            }
    else:
        return {"z": z, "y": y, "mask": mask, "z_mean": z_mean, "z_log_sigma_sq":z_log_sigma_sq }
    ###########################################
...
def main():
    ...
    options_dict["script"] = "train_cvae"
