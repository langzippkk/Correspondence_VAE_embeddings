...
def train_fixed_epochs_external_val(n_epochs, optimizer, train_loss_tensor,
        train_feed_iterator, feed_placeholders, validation_func, save_model_fn,
        save_best_val_model_fn, n_val_interval=1, load_model_fn=None,
        config=None, epoch_offset=0):
    """
    Train a model for a fixed number of epochs with external validation.
    ...
    train_feed_batch_iterator : generator
        ...
        batch.
    feed_placeholders : list of placeholder
        The placeholders that is required for the `train_loss` feeds.
    validation_func : function
        The external validation function. Normally the function uses
        `save_model_fn` to load a model saved at every epoch. It returns a list
        of losses, with the last item used for validation.
    n_val_interval : int
        The number of epochs in between external validation.
    load_model_fn : str
        If provided, initialize session from this file.
    save_model_fn : str
        If provided, save final session to this file.
    save_best_val_model_fn : str
        If provided, save the best validation session to this file.
    ...
    with tf.Session(config=config) as session:
        ...
        for i_epoch in range(n_epochs):
            ...
            if (i_epoch % n_val_interval) == 0:
                validation_loss = validation_func()
                cur_validation_loss = validation_loss[-1]
                # validation_losses = []
                # if not isinstance(validation_loss_tensor, (list, tuple)):
                #     for cur_feed in validation_feed_iterator:
